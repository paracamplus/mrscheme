...
export function installJCanvas ($) {
...
// Symetrize canvas
$.fn.symetrizeCanvas = function(args) {
    var ctx, e,
        params = $.extend({}, jC.prefs, args);
        
    for (e=0; e<this.length; e+=1) {
        ctx = this[e].getContext('2d');
        //window.lastctx = ctx; // DEBUG
        let matrix = ctx.currentTransform || ctx.mozCurrentTransform;
        if ( params.symetrizeX ) {
            ctx.setTransform(  // flipX
                -matrix[0],
                matrix[1],
                matrix[2],
                matrix[3],
...
(quart-de-tour-gauche (triangle -1 -1 1 0 0 -1))
(quarter-turn-right (triangle -1 -1 1 0 0 -1))

(symetrique (quart-de-tour-gauche (triangle -1 -1 1 0 0 -1)))
(symetrique (quarter-turn-right (triangle -1 -1 1 0 0 -1)))
(quart-de-tour-gauche (symetrique (triangle -1 -1 1 0 0 -1)))    ; KO
(quarter-turn-right (symetrique (triangle -1 -1 1 0 0 -1)))      ; KO

(superposition                                                   ; KO
 (triangle -1 -1 1 0 0 -1)
 (quart-de-tour-gauche (symetrique (triangle -1 -1 1 0 0 -1)))
 (symetrique (triangle -1 1 0 0 -1 0.75)) )
